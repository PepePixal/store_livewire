<!-- valores a recibir, que se le pasen desde el componente padre -->
@props([
    'label',
    'name' 
])
...
<div class="flex w-full max-w-xs flex-col gap-1 text-on-surface dark:text-on-surface-dark">
    ...
    <!-- con ({ $atributes }) el input podrá recibir cualquier atributo adicional que se le pase desde el componente padre -->
    <input
        {{ $attributes }}
        id="{{ $name }}" 
        name="{{ $name }}"
